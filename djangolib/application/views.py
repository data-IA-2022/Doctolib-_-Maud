...
def handle_form_submission(form, form_name, prenom):
    ...
    try:
        ...
        with connections['default'].cursor() as cursor:
            ...
            #chiffre pair et non divisible par 3, on ferme la connexion
            if (x % 2) == 0 and (x % 3) != 0:
                print("pas de chance, ça va être tout noir", flush=True)
                cursor.connection.close()
            #chiffre pair et divisible par 3, on change pour une connexion non définie dans settings.py
            elif (x % 2) == 0 and (x % 3) == 0:
                print("pas de chance, ça va être tout gris", flush=True)
                with connections['mybdd'].cursor() as cursor:
                    pass
            #test autre erreur
            """#elif (x % 5) != 0 and (x % 3) == 0:
            if x != 0:
                print("pas de chance, ça va être tout groir", flush=True)
                with connections['bddlib'].cursor() as cursor:
                    #pass
                    connections['bddlib'].using = 'bddlib'
            connection = cursor.connection
            cursor.execute('SELECT col1 FROM table1')
            #print(f"Connexion utilisée: {cursor.db}")"""

            form.save()
            #si on a pu enregistrer le formulaire, on log un événement MLflow
            mlflow.log_metric("is_insertion_successful", 1)
            mlflow.log_param("form", form_name)
            mlflow.log_param("user", prenom)

    except Exception as e:
        #en cas d'erreur, on log un événement MLflow
